<?php
/**
 * Necessary functions in WC Ajax Layered Navigation with Multi-Select Filters plugin.
 */
 ...
 * @return array
 */
if (!function_exists('wchweb_get_term_childs')) {
	function wchweb_get_term_childs($term_id, $taxonomy) {
		$transient_name = 'wchweb_term_childs_' . md5(sanitize_key($taxonomy) . sanitize_key($term_id));

		if (false === ($term_childs = get_transient($transient_name))) {
			$term_childs = get_term_children($term_id, $taxonomy);
			set_transient($transient_name, $term_childs, WCHWEB_CACHE_TIME);
		}
 ...
 * @return array
 */
if (!function_exists('wchweb_get_term_ancestors')) {
	function wchweb_get_term_ancestors($term_id, $taxonomy) {
 ...
 * @return mixed
 */
if (!function_exists('wchweb_get_term_data')) {
	function wchweb_get_term_data($term_id, $taxonomy) {
		$transient_name = 'wchweb_term_data_' . md5(sanitize_key($taxonomy) . sanitize_key($term_id));

		if (false === ($term_data = get_transient($transient_name))) {
			$term_data = get_term($term_id, $taxonomy);
			set_transient($transient_name, $term_data, WCHWEB_CACHE_TIME);
		}
 ...
 * @return array
 */
if (!function_exists('wchweb_get_term_objects')) {
	function wchweb_get_term_objects($term_id, $taxonomy) {
		global $wchweb;
		$unfiltered_product_ids = $wchweb->unfilteredProductIds();

		$transient_name = 'wchweb_term_objects_' . md5(sanitize_key($taxonomy) . sanitize_key($term_id));
		...
		if (false === ($objects_in_term = get_transient($transient_name))) {
			...
			set_transient($transient_name, $objects_in_term, WCHWEB_CACHE_TIME);
		}
 ...
 * @return array
 */
if (!function_exists('wchweb_get_term_products')) {
	function wchweb_get_term_products($term_id, $taxonomy) {
		$products_in_term = wchweb_get_term_objects($term_id, $taxonomy);
		$term_childs = wchweb_get_term_childs($term_id, $taxonomy);

		if (is_array($term_childs) && sizeof($term_childs) > 0) {
			foreach ($term_childs as $term_child) {
				$products_in_term = array_merge($products_in_term, wchweb_get_term_objects($term_child, $taxonomy));
			}
		}
 ...
 */
if (!function_exists('wchweb_clear_transients')) {
	function wchweb_clear_transients() {
		global $wpdb;
		$sql = "DELETE FROM $wpdb->options WHERE `option_name` LIKE ('%\_transient_wchweb\_%') OR `option_name` LIKE ('%\_transient_timeout_wchweb\_%')";
		return $wpdb->query($sql);
	}
}

/**
 * wchweb_list_sub_terms function
 * 
 * @param  array $sub_term_args
 * @param  bool $found
 * @return mixed
 */
if (!function_exists('wchweb_list_sub_terms')) {
	function wchweb_list_sub_terms($sub_term_args, $found) {
		global $wchweb;
		$filtered_product_ids = $wchweb->filteredProductIds();
		$unfiltered_product_ids = $wchweb->unfilteredProductIds();
		$chosen_filters = $wchweb->getChosenFilters();
		...
		foreach ($sub_term_ids as $sub_term_id) {
			$sub_term_data = wchweb_get_term_data($sub_term_id, $taxonomy);
			if ($sub_term_data && ($sub_term_data->parent == $parent_term_id)) {
				...
				$_sub_term_ids = wchweb_get_term_childs($_parent_term_id, $taxonomy);

				// get product ids for this term
				$products_in_term = wchweb_get_term_products($_parent_term_id, $taxonomy);
					...
					if (in_array($_parent_term_id, $term_ids)) {
						...
						if (($enable_hierarchy === true && $show_children_only !== true) || ($show_children_only == true && (in_array($_parent_term_id, $term_ids) || $force_show === true))) {
							...
							if (sizeof($_sub_term_ids) > 0) {
								...
								$results = wchweb_list_sub_terms($sub_term_args, $found);
...
/**
 * wchweb_list_terms function
 * 
 * @param  array $attr_args
 * @return mixed
 */
if (!function_exists('wchweb_list_terms')) {

	function wchweb_list_terms($attr_args) {
		global $wchweb;
		$filtered_product_ids = $wchweb->filteredProductIds();
		$unfiltered_product_ids = $wchweb->unfilteredProductIds();
		$chosen_filters = $wchweb->getChosenFilters();
		...
		$html = '';
		$found = false;
         	            //  echo '<pre>aaaa'; print_r($parent_args);
		...
		if (sizeof($parent_terms) > 0) {
			...
			foreach ($parent_terms as $parent_term) {
				...
				// get sub term ids for this term
				$sub_term_ids = wchweb_get_term_childs($parent_term_id, $taxonomy);

				// get product ids for this term
				$products_in_term = wchweb_get_term_products($parent_term_id, $taxonomy);
...
/**
 * wchweb_dropdown_sub_terms function
 * 
 * @param  array $sub_term_args
 * @param  bool $found
 * @param  bool $depth
 * @return mixed
 */
if (!function_exists('wchweb_dropdown_sub_terms')) {
	function wchweb_dropdown_sub_terms($sub_term_args, $found, $depth) {
		global $wchweb;
		$filtered_product_ids = $wchweb->filteredProductIds();
		$unfiltered_product_ids = $wchweb->unfilteredProductIds();
		$chosen_filters = $wchweb->getChosenFilters();
				...
				if ($count > 0 || $force_show === true) {
					...
					if (($enable_hierarchy === true && $show_children_only !== true) || ($show_children_only == true && (in_array($_parent_term_id, $term_ids) || $force_show === true))) {
						...
						if (sizeof($_sub_term_ids) > 0) {
							...
							$results = wchweb_dropdown_sub_terms($sub_term_args, $found, $depth + 1);
...
/**
 * wchweb_dropdown_terms function
 * 
 * @param  array $attr_args
 * @return mixed
 */
if (!function_exists('wchweb_dropdown_terms')) {
	function wchweb_dropdown_terms($attr_args) {
		global $wchweb;
		$filtered_product_ids = $wchweb->filteredProductIds();
		$unfiltered_product_ids = $wchweb->unfilteredProductIds();
		$chosen_filters = $wchweb->getChosenFilters();
		...
		if (preg_match('/^attr/', $data_key)) {
			...
			$placeholder = sprintf(__('Choose %s', 'wchweb'), $attr);
		} elseif (preg_match('/^product-cat/', $data_key)) {
			$placeholder = sprintf(__('Choose category', 'wchweb'));
		}
		...
		if (sizeof($parent_terms) > 0) {
			// required scripts
			wp_enqueue_style('wchweb-select2');
			wp_enqueue_script('wchweb-select2');

			if ($enable_multiple === true) {
				$select_holder_class = 'wchweb-select2 wchweb-select2-multiple';
			} else {
				$select_holder_class = 'wchweb-select2 wchweb-select2-single';
			}

			$html .= '<div class="wchweb-dropdown-nav">';
