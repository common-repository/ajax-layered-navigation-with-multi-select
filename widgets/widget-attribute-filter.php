<?php
/**
 * WC Ajax Layered Navigation with Multi-Select Filters by Attribute
 */
if (!class_exists('WCHWEB_Attribute_Filter_Widget')) {
	class WCHWEB_Attribute_Filter_Widget extends WP_Widget {
		/**
		 * Register widget with WordPress.
		 */
		function __construct() {
			parent::__construct(
				'wchweb-attribute-filter', // Base ID
				__('WC Ajax Layered Navigation with Multi-Select Filters by Attribute', 'wchweb'), // Name
				array('description' => __('Filter woocommerce products by attribute.', 'wchweb')) // Args
			);
		}
		...
		public function widget($args, $instance) {
			...
			if ($display_type === 'list') {
				$output = wchweb_list_terms($attr_args);
			} elseif ($display_type === 'dropdown') {
				$output = wchweb_dropdown_terms($attr_args);
			}
		...
		public function form($instance) {
			...
			if (sizeof($attribute_taxonomies) > 0) {
				?>
				<label for="<?php echo $this->get_field_id('attr_name'); ?>"><?php printf(__('Attribute', 'wchweb')); ?></label>
				...
				printf(__('No attribute found!', 'wchweb'));
			}
			?>
			</p>
			<p>
				...
				<select class="widefat" id="<?php echo $this->get_field_id('display_type'); ?>" name="<?php echo $this->get_field_name('display_type'); ?>">
					...
					<option value="dropdown" <?php echo ((!empty($instance['display_type']) && $instance['display_type'] === 'dropdown') ? 'selected="selected"' : ''); ?>><?php printf(__('Dropdown', 'wchweb')); ?></option>
				</select>
			</p>
			<p>
				...
				<select class="widefat" id="<?php echo $this->get_field_id('query_type'); ?>" name="<?php echo $this->get_field_name('query_type'); ?>">
					<option value="and" <?php echo ((!empty($instance['query_type']) && $instance['query_type'] === 'and') ? 'selected="selected"' : ''); ?>><?php printf(__('AND', 'wchweb')); ?></option>
					<option value="or" <?php echo ((!empty($instance['query_type']) && $instance['query_type'] === 'or') ? 'selected="selected"' : ''); ?>><?php printf(__('OR', 'wchweb')); ?></option>
				</select>
			</p>
			<p>
				...
				<label for="<?php echo $this->get_field_id('enable_multiple'); ?>"><?php printf(__('Enable multiple filter', 'wchweb')); ?></label>
			</p>
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_count'); ?>"><?php printf(__('Show count', 'wchweb')); ?></label>
			</p>
			<p>
				...
				<label for="<?php echo $this->get_field_id('hierarchical'); ?>"><?php printf(__('Show hierarchy', 'wchweb')); ?></label>
			</p>
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_children_only'); ?>"><?php printf(__('Only show children of the current attribute', 'wchweb')); ?></label>
			</p>
			<?php
		}
...
// register widget
if (!function_exists('wchweb_register_attribute_filter_widget')) {
	function wchweb_register_attribute_filter_widget() {
		register_widget('WCHWEB_Attribute_Filter_Widget');
	}
	add_action('widgets_init', 'wchweb_register_attribute_filter_widget');
}