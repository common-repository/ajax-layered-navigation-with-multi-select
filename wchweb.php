<?php
/**
 * Plugin Name: WC Ajax Layered Navigation with Multi-Select Filters
 * Description: A plugin to Multi-Select Filte woocommerce products with AJAX request.
 * Version: 1.0
 * Author: Rajesh chaudhary
 * Author URI: http://chaudharyweb.com
 * Text Domain: wchweb
 * Domain Path: /languages
 *
 * This program is free software; you can redistribute it and/or modify it under the terms of the GNU 
 * General Public License version 1, as published by the Free Software Foundation.  You may NOT assume 
...
if (!class_exists('WCHWEB')) {
	class WCHWEB
	{
		...
		{
			$this->define('WCHWEB_LOCALE', $this->plugin_slug);
			$this->define('WCHWEB_PATH', $this->pluginPath());
			$this->define('WCHWEB_ASSETS_PATH', $this->assetsPath());
			$this->define('WCHWEB_CACHE_TIME', 60*60*12);
		}
		...
		{
			wp_register_style('wchweb-style', WCHWEB_ASSETS_PATH . 'css/wchweb-styles.css');
			wp_register_style('font-awesome', WCHWEB_ASSETS_PATH . 'css/font-awesome.min.css');
			
			wp_register_script('wchweb-script', WCHWEB_ASSETS_PATH . 'js/scripts.js', array('jquery'), '20120206', true);
			wp_localize_script('wchweb-script', 'wchweb_price_filter_params', array(
				'currency_symbol' => get_woocommerce_currency_symbol(),
				'currency_pos'    => get_option('woocommerce_currency_pos')
			));
			if ($settings = get_option('wchweb_settings')) {
				wp_localize_script('wchweb-script', 'wchweb_params', $settings);
			}
			
			wp_register_style('wchweb-nouislider-style', WCHWEB_ASSETS_PATH . 'css/nouislider.min.css');
			wp_register_script('wchweb-nouislider-script', WCHWEB_ASSETS_PATH . 'js/nouislider.min.js', array ('jquery'), '1.0', true);
			wp_register_script('wchweb-price-filter-script', WCHWEB_ASSETS_PATH . 'js/price-filter.js', array ('jquery'), '1.0', true);

			wp_register_style('wchweb-select2', WCHWEB_ASSETS_PATH . 'css/select2.css');
			wp_register_script('wchweb-select2', WCHWEB_ASSETS_PATH . 'js/select2.min.js', array ('jquery'), '1.0', true);
		}
		...
		{
			add_options_page(__('WC Ajax Layered Navigation with Multi-Select Filters', $this->plugin_slug), __('WC Ajax Layered Navigation with Multi-Select Filters', $this->plugin_slug), 'manage_options', 'wchweb-settings', array($this, 'settingsPage'));
		}
		...
		{
			return array(
				'shop_loop_container'  => '.wchweb-before-products',
				'not_found_container'  => '.wchweb-before-products',
		...
		{
			register_setting('wchweb_settings', 'wchweb_settings', array($this, 'validateSettings'));
		}
		...
		{
			if (!get_option('wchweb_settings')) {
				// check if filter is applied
				$settings = apply_filters('wchweb_settings', $this->defaultSettings());
				update_option('wchweb_settings', $settings);
			}
		}
		...
		{
			if (has_filter('wchweb_settings')) {
				$input = apply_filters('wchweb_settings', $input);
			}
			...
			foreach ($query as $key => $value) {
				...
				if (preg_match('/^attr/', $key) && !empty($query[$key])) {
					...
					foreach ($terms as $term_id) {
						$ancestors = wchweb_get_term_ancestors($term_id, $taxonomy);
						$term_data = wchweb_get_term_data($term_id, $taxonomy);
		...
		public function unfilteredProductIds()
		{
			...
			$transient_name = 'wchweb_unfiltered_product_ids';

			if (false === ($unfiltered_product_ids = get_transient($transient_name))) {
				$unfiltered_product_ids = get_posts($args);
				set_transient($transient_name, $unfiltered_product_ids, WCHWEB_CACHE_TIME);
			}
		...
		public function unfilteredProductsPriceRange()
		{
			...
			$transient_name = 'wchweb_unfiltered_product_price_range';
			...
			if (false === ($unfiltered_products_price_range = get_transient($transient_name))) {
				...
				set_transient($transient_name, $unfiltered_products_price_range, WCHWEB_CACHE_TIME);
			}
		...
		{
			echo '<div class="wchweb-before-products">';
		}
		...
		{
			load_plugin_textdomain('wchweb', FALSE, basename(dirname(__FILE__)) . '/languages/');
		}
		...
		public function needWoocommerce()
		{
			...
			echo '<p>' . __('WC Ajax Layered Navigation with Multi-Select Filters needs WooCommerce plguin to work.', $this->plugin_slug) . '</p>';
			echo '</div>';
		}
		...
		{
			echo '<div class="error">';
			echo '<p>' . __('To use WC Ajax Layered Navigation with Multi-Select Filters update your WooCommerce plugin.', $this->plugin_slug) . '</p>';
			echo '</div>';
		}
		...
		{
			$links[] = '<a href="' . admin_url('options-general.php?page=wchweb-settings') . '">' . __('Settings', $this->plugin_slug) . '</a>';
			return $links;
		}
	}
}
 ...
 */
$GLOBALS['wchweb'] = WCHWEB::instance();